...
class TemporalAnalyzer:
    ...
    def _init_face_detector(self):
        ...
        try:
            ...
            self.mtcnn_detector = MTCNN(keep_all=False, device=DEVICE)
        ...
        # Always initialize Haar Cascade as backup
    ...
    def _detect_face(self, frame):
        ...
        if self.mtcnn_available:
            try:
                boxes, _ = self.mtcnn_detector.detect(frame)
    ...
    def _analyze_landmark_stability(self, frames_data):
        ...
        Face swaps often have unstable tracking
        """
        ...
        for frame_data in frames_data:
            ...
            if len(eye_regions) >= 2:
                # Sort by x position (left eye has smaller x)
                ...
                # Only one eye detected - still track it
        ...
        # Calculate tracking jitter
        left_positions = np.array(left_eye_positions)
        
        # Calculate velocities (first derivative)
        velocities = np.diff(left_positions, axis=0)
        
        # Calculate accelerations (second derivative) - this is the jitter
        accelerations = np.diff(velocities, axis=0)
        
        # Jitter magnitude
        jitter = np.sqrt(np.sum(accelerations**2, axis=1))
        avg_jitter = np.mean(jitter)
        
        # Also check for right eye if available
        if len(right_eye_positions) >= 3:
            right_positions = np.array(right_eye_positions)
            right_velocities = np.diff(right_positions, axis=0)
            right_accelerations = np.diff(right_velocities, axis=0)
            right_jitter = np.sqrt(np.sum(right_accelerations**2, axis=1))
            avg_jitter = (avg_jitter + np.mean(right_jitter)) / 2
        
        # Score based on jitter
        # High jitter = unstable tracking = likely manipulated
        if avg_jitter > 5.0:
            score = 0.8
            interpretation = "High tracking instability - possible face swap"
        elif avg_jitter > 2.0:
            score = 0.5
            interpretation = "Moderate tracking instability"
        else:
            score = 0.2
            interpretation = "Stable tracking - likely authentic"
