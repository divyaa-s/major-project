...
class TemporalAnalyzer:
    ...
    def _analyze_landmark_stability(self, frames_data):
        ...
        Landmark jitter is NORMALIZED by face size (FIX #1)
        """
        ...
        for frame_data in frames_data:
            ...
            # Face diagonal length (normalization factor)
            face_width = x2 - x1
            face_height = y2 - y1
        ...
        face_sizes = np.array(face_sizes[:len(left_positions)])
        ...
        # Normalize jitter by face size (CRITICAL FIX)
        norm_jitter = jitter / face_sizes[2:]  # align with acceleration indices
        avg_jitter = np.mean(norm_jitter)
        ...
        if len(right_eye_positions) >= 3:
            ...
            rj_norm = rj / face_sizes[2:]
            avg_jitter = (avg_jitter + np.mean(rj_norm)) / 2

        # Scoring (NORMALIZED thresholds)
        if avg_jitter > 0.025:
            score = 0.8
            interpretation = "High normalized landmark jitter - likely face swap"
