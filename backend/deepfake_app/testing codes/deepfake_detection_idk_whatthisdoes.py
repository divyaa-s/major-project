...
# CSV logging path
CSV_PATH = os.path.join(settings.BASE_DIR, "deepfake_app/debug_results.csv")
...
def get_model_prediction(model, face, size, name):
    with torch.no_grad():
        ...
        # ────────────────────────────────────────────────
        # Decide which class index corresponds to Fake
        # ────────────────────────────────────────────────
        if name == "convnext":
        ...
        # Optional: keep debug print (you can remove it later)
        if name == "convnext":
...
def plot_radar_summary(freq, gan, color, cnn_ensemble, label, confidence, request):
    try:
        ...
        # Light background circle for reference
...
def generate_gradcam_and_ensemble_predict(request, img_path, true_label=None):
    ...
    # NEW: Priority 0 - High-confidence CNN override (ViT + CNN ensemble both >= 80% Fake)
    vit_prob = probs["vit"]
    if vit_prob >= 0.80 and cnn_ensemble >= 0.80 and forensic >= 0.40:
        ...
        label = "Fake"
        confidence = cnn_ensemble  # Use ensemble as confidence
        ...
        if abs(ensemble - FAKE_THRESHOLD) < 0.10:
            ...
            if ensemble >= FAKE_THRESHOLD and forensic < 0.45:
                ...
                if ensemble >= FAKE_THRESHOLD:
                    ...
                    logger.warning(f"Borderline Fake call (forensic={forensic:.3f})")
                else:
                    label = "Real"
                    confidence = 1 - ensemble
                    decision_source = "ensemble_real_borderline"
                    logger.warning(f"Borderline Real call (forensic={forensic:.3f})")
        
        # Relaxed real-safety veto
        elif ensemble < 0.55 and forensic < 0.52 and not portrait_gan_suspect:
    ...
    # GRAD-CAM: Use consensus-based winner (already correct in your code)
    ...
    try:
        ...
        logger.debug(f"[CSV DEBUG] Target path: {CSV_PATH}")
        logger.debug(f"[CSV DEBUG] Directory: {csv_dir}")
        logger.debug(f"[CSV DEBUG] Dir exists? {os.path.exists(csv_dir)}")

        if not os.path.exists(csv_dir):
            os.makedirs(csv_dir, exist_ok=True)
            logger.info(f"[CSV DEBUG] Created missing directory")
        ...
        with open(CSV_PATH, 'a', newline='', encoding='utf-8') as f:
            ...
            if not file_exists:
                ...
                logger.info(f"[CSV DEBUG] Wrote header")
            ...
            row = [
                ...
                winner["model_name"] if winner else "N/A",
        ...
        logger.info(f"[CSV DEBUG] Successfully appended row for {os.path.basename(img_path)}")

    except Exception as csv_err:
        logger.error(f"[CSV CRITICAL ERROR] {str(csv_err)}")
        logger.error(f"[CSV CRITICAL ERROR] Path attempted: {CSV_PATH}")
