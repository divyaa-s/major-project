...
# CSV logging path
CSV_PATH = os.path.join(settings.BASE_DIR, "debug_results.csv")
...
# RADAR PLOT FUNCTION
# -------------------------
def plot_radar_summary(freq, gan, color, watermark, cnn_ensemble,
                      label, confidence, request, watermark_override=False):
    try:
        categories = [
            'Frequency',
            'GAN fingerprint',
        ...
        values = [freq, gan, color, watermark, cnn_ensemble]
        ...
        # Style: watermark override = dashed + darker red
        line_style = '--' if watermark_override else '-'
        color_main = '#c0392b' if watermark_override else '#e74c3c'

        ax.plot(angles, values, color=color_main,
                linewidth=2.5, linestyle=line_style)
        ax.fill(angles, values, color=color_main, alpha=0.25)

        ax.fill(angles, [1.0] * len(angles), color='gray', alpha=0.06)
        ...
        title_suffix = " (watermark override)" if watermark_override else ""
        ax.set_title(
            f"Signal Profile — {label} ({confidence:.1%}){title_suffix}",
            size=13, y=1.12
        )
        ...
        return request.build_absolute_uri(settings.MEDIA_URL + "plots/" + radar_filename)
...
def generate_gradcam_and_ensemble_predict(request, img_path, true_label="Unknown"):
    ...
    gan=gan,
    color=color,
    watermark=watermark_prob,
    cnn_ensemble=cnn_ensemble,
    label=label,
    confidence=confidence,
    request=request,
    watermark_override=(decision_source == "watermark_high_conf_override")
)
    ...
    decision_trace = {
        "watermark": {
            ...
            "zone": (
                "strong" if watermark_prob >= 0.50 else
                "weak" if watermark_prob >= 0.12 else
                "clean"
            ),
            "override_triggered": decision_source == "watermark_high_conf_override"
        },
        "cnn": {
            "ensemble_score": round(cnn_ensemble, 4),
            "strong_fake": cnn_ensemble >= 0.90
        },
        "forensic": {
            "score": round(forensic, 4),
            "portrait_gan_boost": portrait_gan_suspect
        },
        "final_decision": {
            "label": label,
