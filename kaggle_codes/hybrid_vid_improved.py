...
MODEL_CONFIGS = {
    "efficientnet_b3": {
        "path":       "/kaggle/input/models/divyaasriram/new-models/pytorch/default/1/new_effb3_finetuned.pth",
        "input_size": (300, 300),
    ...
    "xception": {
        "path":       "/kaggle/input/models/divyaasriram/new-models/pytorch/default/1/new_xception_finetuned.pth",
        "input_size": (299, 299),
    ...
    "vit": {
        "path":       "/kaggle/input/models/divyaasriram/new-models/pytorch/default/1/new_vit_finetuned.pth",
        "input_size": (224, 224),
    ...
    "convnext": {
        "path":       "/kaggle/input/models/divyaasriram/new-models/pytorch/default/1/new_convnext_finetuned.pth",
        "input_size": (224, 224),
...
# BiLSTM model paths
# ---------------------------------------------------------------------------
BILSTM_CHECKPOINT  = "/kaggle/input/models/divyaasriram/final-bilstm/pytorch/default/1/bilstm_v2_best.pth"
BILSTM_NORM_STATS  = "/kaggle/input/models/divyaasriram/final-bilstm/pytorch/default/1/norm_stats.pt"
...
class ImprovedHybridAnalyzer:
    def __init__(self, num_keyframes=100, temporal_max_frames=30, temporal_skip=2,
        ...
        # NEW (CORRECT):
        meta_learner_path = '/kaggle/input/models/divyaasriram/video-xgboost/pytorch/default/1/video_meta_learner_v2.pkl'
    ...
    def _analyze_keyframes_with_cnns(self, keyframes):
        if not self.models:
            ...
            return {"frame_predictions": [], "ensemble": {"avg_fake_probability": 0.5, "per_model_avg": {}, "frames_analyzed": 0}}

        frame_predictions = [{"frame_index": kf["frame_index"], "model_predictions": {}} for kf in keyframes]
        
        # 1. Pre-process and Batch Tensors (CPU/RAM side)
        batch_inputs = {name: [] for name in self.models.keys()}
        
        for kf in tqdm(keyframes, desc="📦 Batching Tensors", leave=False):
            face_img = kf["face"]
            for name, model_info in self.models.items():
                try:
                    ...
                    x = self._preprocess_face(face_color, config["input_size"], config["norm_mean"], config["norm_std"])
                    batch_inputs[name].append(x)
                except Exception as e:
                    logger.warning(f"Failed to preprocess for {name}: {e}")
                    # Append a zero-tensor fallback so batch sizes match
                    fallback = torch.zeros((1, 3, config["input_size"][0], config["input_size"][1])).to(DEVICE)
                    batch_inputs[name].append(fallback)

        # 2. FP16 GPU BATCH INFERENCE (The massive speedup)
        # Using Mixed Precision cuts memory in half and doubles speed on Kaggle GPUs
        import torch.nn.functional as F
        with torch.no_grad(), torch.autocast(device_type=DEVICE, dtype=torch.float16):
            for name, model_info in tqdm(self.models.items(), desc="🚀 GPU Batch Inference", leave=False):
                if not batch_inputs[name]: continue
                
                # Combine all frames into one massive tensor block! [N, 3, H, W]
                batch_tensor = torch.cat(batch_inputs[name], dim=0).to(DEVICE)
                
                try:
                    output = model_info["model"](batch_tensor)
                    
                    if model_info["config"].get("num_classes", 1) == 1:
                        probs = torch.sigmoid(output).squeeze(1).tolist()
                        if not isinstance(probs, list): probs = [probs] # Handle single frame case
                        fake_probs = [1.0 - p for p in probs]
                    else:
                        fake_probs = F.softmax(output, dim=1)[:, 1].tolist()
                        
                    # Map the bulk results back to individual frames
                    for i, fp in enumerate(fake_probs):
                        frame_predictions[i]["model_predictions"][name] = float(fp)
                        
                except Exception as e:
                    logger.error(f"Batch inference failed for {name}: {e}")
                    for i in range(len(keyframes)):
                        frame_predictions[i]["model_predictions"][name] = 0.5
