...
class TemporalAnalyzer:
    ...
    def _init_face_detector(self):
        try:
            ...
            logger.info("✅ MTCNN initialized (CPU)")
        except Exception as e:
            cascade_path = cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
            self.face_detector = cv2.CascadeClassifier(cascade_path)
            self.face_detector_type = "haar"
            logger.info("✅ Haar Cascade initialized (MTCNN fallback)")
    ...
    def _analyze_blinks(self, frames_data, fps=25.0, total_frames=None):
        ...
        # Use real video duration instead of hardcoded * 2.0
        if total_frames and fps and fps > 0:
            duration_seconds = total_frames / fps
        else:
            duration_seconds = len(frames_data) * 2.0  # old fallback
    ...
    def analyze_from_frames(self, frames_data, video_path):
        """
        Skip video reading — use pre-extracted frames directly.
        Called by ImprovedHybridAnalyzer after single-pass extraction.
        """
        if not frames_data:
            return {"error": "No faces detected in video"}

        # Get video metadata for blink calculation
        cap         = cv2.VideoCapture(video_path)
        fps         = cap.get(cv2.CAP_PROP_FPS)
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        cap.release()
        ...
        blink_result        = self._analyze_blinks(frames_data, fps, total_frames)
        optical_flow_result = self._analyze_optical_flow(frames_data)
        landmark_result     = self._analyze_landmark_stability(frames_data)
